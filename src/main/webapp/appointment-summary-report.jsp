...
while (entryIterator2.hasNext()) {
	...
	if (entryIterator2.hasNext()) {
		out.print(", ");
					...
					backgroundColor : [ 'rgba(255, 99, 132, 0.7)', // Requested - Red
		...
		<script>
			const dynamicCountryAppointmentData = {
				labels :
		<%=new ArrayList<>(countryAppointmentData.keySet()).stream().map(country -> "'" + country + "'")
		.collect(Collectors.joining(", ", "[", "]"))%>
			,
				datasets : [ {
					data :
		<%=new ArrayList<>(countryAppointmentData.values()).toString()%>
			,
					backgroundColor : generateRandomColors(
		<%=countryAppointmentData.size()%>
			),
					borderWidth : 1
				} ]
			};
			...
			const ctxCountryPie = document.getElementById(
					'countryAppointmentsPieChart').getContext('2d');
